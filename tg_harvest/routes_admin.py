...
class AdminRoutesHandler:
    ...
    def api_admin_job_create_harvest(self):
        ...
        if self.admin_has_any_active_job_fn():
        ...
        if len(target) > self.admin_harvest_target_max_len:
            return jsonify({"ok": False, "error": f"target 长度不能超过 {self.admin_harvest_target_max_len}"}), 400

        job = self.admin_job_create_fn("harvest", target_chat_id=None, target_label=target)
    ...
    def api_admin_job_create_update(self):
        ...
        if is_all_scope:
            job = self.admin_job_create_fn("update", target_chat_id=None, target_label="全部群聊")
            job_id = str(job.get("job_id") or "")
            ...
            try:
                with closing(self.get_conn_fn()) as conn:
                    chat_brief = self.admin_get_chat_brief_fn(conn, chat_id)
            except sqlite3.Error:
            ...
            job, existing_job = self.admin_create_chat_job_if_absent_fn("update", chat_id=chat_id, target_label=chat_title)
    ...
    def api_admin_job_create_delete(self):
        ...
        chat_title = str(chat_brief["chat_title"])
        job, existing_job = self.admin_create_chat_job_if_absent_fn("delete", chat_id=chat_id, target_label=chat_title)
        if existing_job is not None:
            return jsonify({"ok": False, "error": "该目标已有进行中的任务", "existing_job": existing_job}), 409
        job_id = str(job.get("job_id") or "")
    ...
    def api_admin_job_create_cleanup(self):
        ...
        if len(keyword) > self.admin_cleanup_keyword_max_len:
            return jsonify({"ok": False, "error": f"keyword 长度不能超过 {self.admin_cleanup_keyword_max_len}"}), 400
        ...
        job = self.admin_job_create_fn("cleanup", target_chat_id=chat_id, target_label=target_label)
        job_id = str(job.get("job_id") or "")
    ...
    def api_admin_job_create_cleanup_empty(self):
        ...
        job = self.admin_job_create_fn("cleanup_empty", target_chat_id=chat_id, target_label=target_label)
        job_id = str(job.get("job_id") or "")

        self.admin_job_append_log_fn(job_id, "已接收无文本媒体清理请求")
        scope_label = {"all": "全部数据", "chat": "当前群组"}.get(scope, scope)
        self.admin_job_append_log_fn(job_id, f"作用范围：{scope_label}")
        chat_suffix = '' if chat_id is None else f' ({chat_id})'
        self.admin_job_append_log_fn(job_id, f"目标：{target_label}{chat_suffix}")
...
def register_admin_routes(app, **kwargs) -> None:
    handler = AdminRoutesHandler(**kwargs)
    ...
    app.post("/api/admin/jobs/cleanup-empty")(handler.api_admin_job_create_cleanup_empty)
