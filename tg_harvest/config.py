...
def _load_raw_config_values() -> dict:
    return {
        ...
        # 未配置时为 0；需要连接 Telegram 时必须通过环境变量提供真实值。
        ...
        # 既可传相对路径也可传绝对路径；Web 与 CLI 入口统一使用该配置值。
        ...
        # 用途：在广告过滤系统启用时，用于规则打分模型识别推广/广告信息，分数达到阈值视为命中。
        # 允许为 0；当 disable_promo_filter=1 时，该参数不会参与新消息判定。
        "promo_score_threshold": _env_int("TG_PROMO_SCORE_THRESHOLD", 0),
        # 是否彻底关闭广告过滤系统（环境变量：TG_DISABLE_PROMO_FILTER，0/1）。
        # 用途：开启后，新处理的消息/媒体组不再做广告判定，也不会进入广告去重链路。
        # 注意：该开关不会自动清理数据库中历史 is_promo / dedupe_eligible 标记。
        # 规范化时仅 1 视为开启，其他值一律按 0 关闭处理。
        ...
        # SQLite 页面缓存大小 (MB)（环境变量：TG_SQLITE_CACHE_MB）。
        # 用途：控制单连接 page cache 上限；值越大通常读写更稳，但会增加进程内存占用。
        "sqlite_cache_mb": _env_int("TG_SQLITE_CACHE_MB", 512),
        # SQLite 内存映射大小 (MB)（环境变量：TG_SQLITE_MMAP_MB）。
        # 用途：控制 SQLite mmap 上限；为 0 表示关闭内存映射。
        "sqlite_mmap_mb": _env_int("TG_SQLITE_MMAP_MB", 1024),
        # 任务管理：最大保留任务数（环境变量：TG_ADMIN_JOB_MAX_COUNT）
        "admin_job_max_count": _env_int("TG_ADMIN_JOB_MAX_COUNT", 100),
        # 任务管理：单个任务最大日志行数（环境变量：TG_ADMIN_JOB_LOG_MAX_LINES）
        "admin_job_log_max_lines": _env_int("TG_ADMIN_JOB_LOG_MAX_LINES", 5000),
        # 任务管理：全部群组更新时的并发数量（环境变量：TG_ADMIN_UPDATE_CONCURRENCY）
        "admin_update_concurrency": _env_int("TG_ADMIN_UPDATE_CONCURRENCY", 10),
        # 启动时是否强制执行一次 FTS 全量同步修复（环境变量：TG_FORCE_HEAL_FTS，0/1）。
        # 用途：Web 与 CLI 入口在建表后都会读取该开关；开启后会从 messages 全量重建 messages_fts 内容。
        "force_heal_fts": _env_int("TG_FORCE_HEAL_FTS", 0),
        # 后台管理密码（环境变量：TG_ADMIN_PASSWORD）
        "admin_password": _env_str("TG_ADMIN_PASSWORD", ""),
        # 后台自动注销时间（秒）（环境变量：TG_ADMIN_SESSION_EXPIRY）
