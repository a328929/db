(function () {
  ...
  function setupAutoLogout(seconds) {
    ...
    console.log('[Auth] 将在 ' + seconds + ' 秒后自动注销');
  ...
  async function handleCleanupEmptyClick(elements) {
    ...
    if (!confirmAction(elements, confirmText, '已取消无文本媒体清理操作')) {
      return;
    }
    ...
    try {
      await createJobAndStartPolling(elements, {
        ...
        requestPayload: buildScopeRequestPayload(target, 'chat_id 参数非法'),
        successMessage: '无文本媒体清理任务已创建：{jobId}，范围：' + scopeLabel
      });
    } catch (error) {
  ...
  async function handleDeleteDataClick(elements) {
    ...
    try {
      await createJobAndStartPolling(elements, {
        ...
        requestPayload: {
          chat_id: getRequiredIntegerChatId(target, '当前目标 ID 非法')
        },
