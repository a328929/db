(() => {
  ...
  function _buildMessageElement(item) {
      ...
      if (item.message_id === state.targetMsgId) {
          li.classList.add("is-target"); // 视觉上会有不同的背景色
      }
      if (item.is_promo) li.classList.add("is-promo");

      // 核心修改：使用 <h3> 标题包裹文本，方便读屏软件按标题快捷浏览
      ...
      metaFooter.style.marginTop = "8px";
      ...
      if (item.link) {
          ...
          if (mediaParts.length > 0) linkSpan.style.marginLeft = "12px";
