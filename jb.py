...
class AppConfig:
    ...
    def load(cls) -> "AppConfig":
        return cls(
            ...
            rescan_tail_ids=_env_int("TG_RESCAN_TAIL_IDS", 4000000),
...
PROMO_KEYWORDS = [
    ...
    "私聊", "联系", "联系方式", "联系客服", "客服", "商务", "酒馆", "体验", "商务合作", "合作", "代理", "投稿", "推广",
...
CTA_WORDS = [
    "点击", "加入", "进群", "加群", "私信", "联系我", "车队", "酒馆", "体验", "联系客服", "搜索", "满足你的", "投稿", "合作",
...
def search_keyword_fts(conn: sqlite3.Connection, keyword_query: str, limit: int = 20):
    cur = conn.cursor()

    if not table_exists(conn, "messages_fts"):
        sql = """
        SELECT m.chat_id, c.chat_title, m.message_id, m.msg_date_text, m.msg_type, m.link, m.content
        FROM messages m
        LEFT JOIN chats c ON c.chat_id = m.chat_id
        WHERE m.content LIKE ?
        ORDER BY m.msg_date_ts DESC
        LIMIT ?
        """
        cur.execute(sql, [f"%{keyword_query}%", limit])
        return cur.fetchall()
    ...
    LIMIT ?
    """
    cur.execute(sql, [keyword_query, limit])
    return cur.fetchall()
...
def search_by_filters(
    ...
    only_promo: Optional[bool] = None,
    dedupe_eligible: Optional[bool] = None,
    grouped_only: Optional[bool] = None,
    ...
    SELECT
        m.*, c.chat_title,
        mm.media_kind, mm.file_name, mm.mime_type, mm.file_size, mm.width, mm.height, mm.duration_sec
    FROM messages m
    LEFT JOIN chats c ON c.chat_id = m.chat_id
    ...
    if msg_type:
        sql.append("AND m.msg_type=?")
        params.append(msg_type)
    if only_promo is not None:
        sql.append("AND m.is_promo=?")
        params.append(1 if only_promo else 0)
    if dedupe_eligible is not None:
        sql.append("AND m.dedupe_eligible=?")
        params.append(1 if dedupe_eligible else 0)
    if grouped_only is True:
        sql.append("AND m.grouped_id IS NOT NULL")
    elif grouped_only is False:
        sql.append("AND m.grouped_id IS NULL")
    if min_file_size is not None:
        sql.append("AND COALESCE(mm.file_size, 0) >= ?")
        params.append(int(min_file_size))
    if max_file_size is not None:
        sql.append("AND COALESCE(mm.file_size, 0) <= ?")
        params.append(int(max_file_size))

    sql.append("ORDER BY m.msg_date_ts DESC LIMIT ?")
    params.append(limit)
